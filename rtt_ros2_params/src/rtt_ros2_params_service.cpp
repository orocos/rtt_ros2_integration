...
#include "rtt_ros2_params/rtt_ros2_params_service.hpp"
...
RosParamService::RosParamService(RTT::TaskContext * owner)
: RTT::Service("rosparam", owner)
{
  ...
  doc(
    "RTT Service for getting and setting ROS 2 parameters directly or to synchronize them with "
    "properties of the owning RTT component");

  addOperation(
    "getParameter", &RosParamService::getParameter, this, RTT::ClientThread)
  .doc("Gets the value of a ROS 2 parameter")
  .arg("name", "Name of the parameter to retrieve");

  addOperation(
    "setParameter", &RosParamService::setParameter, this, RTT::ClientThread)
  ...
  if (nullptr != owner) {
    addOperation(
      "loadProperty", &RosParamService::loadProperty, this, RTT::ClientThread)
    ...
    addOperation(
      "storeProperty", &RosParamService::storeProperty, this, RTT::ClientThread)
...
RosParamService::~RosParamService() = default;

rclcpp::ParameterValue RosParamService::getParameter(const std::string & name)
{
  RTT::Logger::In in(getName());
...
bool RosParamService::setParameter(
...
bool RosParamService::loadProperty(
...
bool RosParamService::storeProperty(
  const std::string & property_name,
  const std::string & param_name)
{
  RTT::Logger::In in(getName());
